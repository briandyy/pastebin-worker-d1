...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function updateButtons() {
    ...
    if (!pasteNotEmpty) {
      ...
      disableSubmitButton(
        `The customized URL should satisfy regex ${NAME_REGEX}`,
      )
    ...
    if (adminUrlValid) {
      ...
      submitErrMsg.text(
        `The admin URL should start with “${base_url}” and contain a colon`,
      )
    }
  }
  ...
  function handleError(error) {
    ...
    let statusText =
      error.statusText === "error" ? "Unknown error" : error.statusText
