...
test("content disposition without specifying filename", async () => {
  ...
  expect(
    (await workerFetch(url)).headers.get("Content-Disposition"),
  ).toStrictEqual("inline")
  expect(
    (await workerFetch(`${url}?a`)).headers.get("Content-Disposition"),
  ).toStrictEqual("attachment")

  expect(
    (await workerFetch(`${url}/${filename}`)).headers.get(
      "Content-Disposition",
    ),
  ).toStrictEqual(`inline; filename*=UTF-8''${filename}`)
  expect(
    (await workerFetch(`${url}/${filename}?a`)).headers.get(
      "Content-Disposition",
    ),
...
test("content disposition with specifying filename", async () => {
  ...
  expect(
    (await workerFetch(`${url}/${altFilename}`)).headers.get(
      "Content-Disposition",
    ),
  ).toStrictEqual(`inline; filename*=UTF-8''${altFilename}`)
  expect(
    (await workerFetch(`${url}/${altFilename}?a`)).headers.get(
      "Content-Disposition",
    ),
